...
At each iteration, $t$, BSSMC updates a sample of $N$ particles, ${\theta_i^t}_{i=1}^N$, to approximate the posterior distribution, $p(\theta|y)$.

Resample particles according to weights, $w_i^t \propto p(y|\theta_i^t)$.

Propagate particles to the next iteration, $\theta_i^{t+1} \sim p(\theta|\theta_i^t,y)$.

Update weights, $w_i^{t+1} \propto p(y|\theta_i^{t+1})$.
